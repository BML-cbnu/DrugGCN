...
def ndcgk_row_loss_gradient(W, X, Y, k, Notnan, njobs):
    ...
    if step == 0:
        step = 1
    results = Parallel(n_jobs=njobs)(delayed(ndcgk_block_loss_gradient)(i, F[i:i+step], Y[i:i+step], k, Notnan[i:i+step]) for i in range(0, n, step))    
...
def ndcgk_row_loss(W, X, Y, k, Notnan, njobs):
    ...
    if step == 0:
        step = 1    
    results = Parallel(n_jobs=njobs)(delayed(ndcgk_block_loss)(F[i:i+step], Y[i:i+step], k, Notnan[i:i+step]) for i in range(0, n, step))    
    for li in results:
        l += li
